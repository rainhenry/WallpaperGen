...
void CQtAI::run()
{
    ...
    while(1)
    {
        ...
        std::string sdxl_prompt_str = this->sdxl_prompt.toStdString();
        int sdxl_resolution_val = this->sdxl_resolution;
        std::string sdxl_output_file_str = this->sdxl_output_file.toStdString();
        cmd_mutex.unlock();
        ...
        //  当为文字生成壁纸命令
        else if(now_cmd == EAIcmd_ExTextToWallpaper)
        {
            //  设置忙
            cmd_mutex.lock();
            this->cur_st = EAISt_Busy;
            ...
            QElapsedTimer run_time;
            run_time.start();
            py_ai->Text_To_Wallpaper(sdxl_prompt_str.c_str(),
                                     sdxl_resolution_val,
                                     sdxl_output_file_str.c_str()
                                    );
            qint64 run_time_ns = run_time.nsecsElapsed();

            //  发出操作完成
            emit send_text_to_wallpaper_finish(run_time_ns);
...
//  执行一次文字生成壁纸文件
void CQtAI::ExTextToWallpaper(QString prompt, int resolution, QString output_file)
{
    if(GetStatus() == EAISt_Busy) return;

    cmd_mutex.lock();
    this->cur_cmd = EAIcmd_ExTextToWallpaper;
    this->sdxl_prompt = prompt;
    this->sdxl_resolution = resolution;
    this->sdxl_output_file = output_file;
    this->cur_st = EAISt_Busy;    //  设置忙
    cmd_mutex.unlock();
